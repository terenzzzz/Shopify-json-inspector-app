...
function generateLocalHmac(config) {
    return async (params, signator = 'admin') => {
        ...
        return utils.createSHA256HMAC(config.apiSecretKey, queryString, types$3.HashFormat.Hex);
    };
}
...
function validateHmacFromRequestFactory(config) {
    return async function validateHmacFromRequest({ type, rawBody, webhookType, ...adapterArgs }) {
        ...
        const hmacHeaderName = webhookType
            ? types$1.WEBHOOK_HEADER_NAMES[webhookType].hmac
            : types$2.ShopifyHeader.Hmac;
        const hmac = headers.getHeader(request.headers, hmacHeaderName);
        if (!hmac) {
            return fail(types.ValidationErrorReason.MissingHmac, type, config);
        }
        const validHmac = await validateHmacString(config, rawBody, hmac, types$3.HashFormat.Base64);
        if (!validHmac) {
